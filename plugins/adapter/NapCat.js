...
Bot.adapter.push(
  new (class NapCatAdapter {
    ...
    deleteGroupFile(data, file_id) {
      Bot.makeLog("info", `删除群文件：${file_id}`, `${data.self_id} => ${data.group_id}`, true)
      return data.bot.sendApi("delete_group_file", { group_id: data.group_id, file_id })
    }
    ...
    async getCookies(data, domain) {
      ...
      const cookies = result?.data?.cookies ?? result?.cookies
      if (cookies) data.bot.cookies[domain] = cookies
      return result
    }
    ...
    // 获取 RKey 服务器
    getRkeyServer(data) {
      return data.bot.sendApi("get_rkey_server", {})
    }
    ...
    // 获取 AI 语音记录
    getAiRecord(data, params) {
      return data.bot.sendApi("get_ai_record", params)
    }

    // 获取模型显示
    getModelShow(data) {
      return data.bot.sendApi("_get_model_show", {})
    }
    ...
    // 重启
    setRestart(data, params) {
      return data.bot.sendApi("set_restart", params)
    }
    ...
    // 清理缓存
    cleanCache(data) {
      return data.bot.sendApi("clean_cache", {})
    }

    // 退出 Bot
    botExit(data) {
      return data.bot.sendApi("bot_exit", {})
    }
    ...
    async connect(data, ws) {
      Bot[data.self_id] = {
        ...
        getAiRecord: params => this.getAiRecord(data, params),
        getMiniAppArk: params => this.getMiniAppArk(data, params),

        setRestart: params => this.setRestart(data, params),
        botExit: this.botExit.bind(this, data),
        cleanCache: this.cleanCache.bind(this, data),

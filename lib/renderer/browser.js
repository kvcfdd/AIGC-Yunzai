...
class Browser {
   ...
   */
  async getBrowser() {
    if (this.taskDepth <= 0) throw new Error("[Browser] 禁止直连 Chromium")
    if (this.restarting) {
  ...
  /**
   * 统一任务入口：全局并发控制
   * 排队执行 fn，计入重启阈值与空闲回收逻辑
   * 已在任务回调内再次 runTask 时直接执行不排队，避免并发占满时自我死锁
   * @param fn 要执行的任务
   */
  runTask(fn) {
    if (this.taskDepth > 0) return Promise.resolve().then(fn)

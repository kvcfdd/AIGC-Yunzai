...
export default class Yunzai extends EventEmitter {
  ...
  serverAuth(req) {
    ...
    for (const i in cfg.server.auth) {
      if (req.headers[i.toLowerCase()] === cfg.server.auth[i] || req.query[i] === cfg.server.auth[i]) continue
  ...
  serverStatus(req) {
    req.res.type("json")
    req.res.send(JSON.stringify(process.report.getReport()).replace(/(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)/g, "[IPv4]"))
  }
  ...
  wsConnect(req, socket, head) {
    if (this.stat.online !== 2) return this.once("online", this.wsConnect.bind(this, req, socket, head))
  ...
  async serverLoad(https) {
    ...
    for (const name of [404, "timeout"]) this.fileToUrl(`resources/http/File/${name}.jpg`, { name, time: false, times: false })
  }
  ...
  async run() {
    ...
    this.stat.online = 1
    await init()
    const aigc = await import("./aigc/index.js")
    this.aigc = aigc
    await aigc.mcp.init()
    await aigc.agent.mcp.init()
  ...
  async fileToUrl(file, opts = {}) {
    ...
    file = (typeof file === "object" && !Buffer.isBuffer(file) && { ...file }) || (await util.fileType({ file, name }, { http: true }))
  ...
  get gml() {
    const map = new Map()
    for (const bot_id of this.uin) for (const [id, i] of this.bots[bot_id].gml || []) map.set(id, Object.assign(new Map(i), { bot_id }))
    return map
  }
  ...
  async redisExit() {
    if (typeof redis !== "object" || !redis) return false
    try {
      redis.close()
      return true
    } catch (err) {
      util.makeLog("error", ["存储关闭失败", err], "存储")
      return false
    }
  }

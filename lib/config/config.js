...
export default new (class Cfg {
  ...
  initCfg() {
    ...
    if (fs.existsSync(path)) {
      ...
      for (const i of filesDef) if (!files.includes(i)) fs.copyFileSync(`${pathDef}${i}`, `${path}${i}`)
  ...
  /**
   * 写入用户配置到 config/config/{name}.yaml，自动失效缓存
   * @param name 配置文件名
   * @param key  配置键，支持点路径
   * @param value 值
   */
  setConfig(name, key, value) {
    const file = `config/config/${name}.yaml`
    try {
      ...
      Bot.makeLog("error", ["写入配置文件", file, "错误", err], "Config")
    }
  }

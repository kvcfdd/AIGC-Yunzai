...
class PluginsLoader {
  ...
  async load(isRefresh = false) {
    ...
    await Promise.allSettled(
      files.map(async file => {
        if ((await Bot.sleep(cfg.bot.plugin_load_timeout * 1000, this.importPlugin(file, packageErr))) === Bot.sleepTimeout) Bot.makeLog("error", `插件加载超时 ${logger.red(file.name)}`, "Plugin")
      }),
    )
  ...
  async importPlugin(file, packageErr) {
    ...
    try {
      ...
      for (const i of await Promise.allSettled(pluginArray)) if (i?.status && i.status !== "fulfilled") Bot.makeLog("error", [`插件加载错误 ${logger.red(file.name)}`, i], "Plugin")
    } catch (error) {
  ...
  async loadPlugin(file, p) {
    ...
    plugin = new p()
    /** 初始化正则表达式 */
    if (plugin.rule) for (const i of plugin.rule) if (!(i.reg instanceof RegExp)) i.reg = new RegExp(i.reg)
  ...
  /** 以正则指令驱动插件规则：
   *  返回 true = 有插件命中，false = 无匹配。
   *  跳过计数/黑白名单/冷却/only_reply_at 门控/上下文 hook，
   *  排除 AIGC 兜底插件防止递归。
   *  @param {object} src 真实聊天事件
   *  @param {string} msg 指令文本 */
  async dealCommand(src, msg) {
    const e = { ...src }
    e.msg = this.dealText(String(msg ?? ""))
    if (!e.msg) return false

    // 浅拷贝丢失 isSr/isGs getter，重新定义
    Object.defineProperty(e, "isSr", {
      get: () => e.game === "sr",
      set: v => (e.game = v ? "sr" : "gs"),
    })
    ...
    for (const i of this.priority) {
      /** 排除 AIGC 兜底 */
      if (i.name === "AIGC") continue
      if (this.checkDisable({ ...i.plugin, e }) && this.filtEvent(e, i.plugin)) priority.push(i)
    }
  ...
  dealEvent(e) {
      ...
      for (const i of e.message) {
        switch (i.type) {
          ...
          case "image":
            ...
            break
          case "video":
            e.video = [i.url || i.file]
            break
          case "record":
          case "audio":
            e.audio = [i.url || i.file]
            break
          case "at":
    ...
    e.only_reply_at = this.onlyReplyAt(e)
  }
  ...
  async count(e, type, msg) {
    if (cfg.bot.msg_type_count) for (const i of Array.isArray(msg) ? msg : [msg]) await this.saveCount(e, `${type}:${i?.type || "text"}`)
  ...
  createTask() {
    ...
    for (const i of this.task) {
      ...
      i.job = schedule.scheduleJob(i.cron.split(/\s+/).slice(0, 6).join(" "), this.startTask.bind(this, name, i))
    }
  }
  ...
  checkLimit(e) {
    /** 禁言中 */
    if (e.group && (e.group.mute_left > 0 || (e.group.all_muted && !e.group.is_admin && !e.group.is_owner))) return false
  ...
  checkBlack(e) {
    ...
    /** 黑名单用户 */
    if (other.blackUser?.length && other.blackUser.includes(Number(e.user_id) || String(e.user_id))) return false
    /** 白名单用户 */
    if (other.whiteUser?.length && !other.whiteUser.includes(Number(e.user_id) || String(e.user_id))) return false

    if (e.group_id) {
      /** 黑名单群 */
      if (other.blackGroup?.length && other.blackGroup.includes(Number(e.group_id) || String(e.group_id))) return false
      /** 白名单群 */
      if (other.whiteGroup?.length && !other.whiteGroup.includes(Number(e.group_id) || String(e.group_id))) return false
    }

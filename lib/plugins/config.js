...
export default async function makeConfig(name, config = {}, keep = {}, opts = {}) {
  ...
  const configSave = util.debounce(typeof opts.replacer === "function" ? async () => fs.writeFile(configFile, await opts.replacer(YAML.stringify(config)), "utf8") : () => fs.writeFile(configFile, YAML.stringify(config), "utf8"))
  ...
  if (typeof opts.watch === "boolean" ? opts.watch : cfg.bot.file_watch) ret.watcher = chokidar.watch(configFile).on("change", _.debounce(watcher.bind(ret), 5000))

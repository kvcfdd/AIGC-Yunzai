  ...
  parameters: {
    ...
    properties: {
      ...
      payload: {
        type: "string",
        description: `内容:
- image: URL 或本地路径，多图用英文逗号分隔，如 "url1,url2,url3"，pixiv 图片 (i.pximg.net) 会自动添加 Referer + 走代理绕过防盗链
  ...
  execute: async (args, ctx) => {
    ...
    if (type === "video") {
      if (!isURL(String(payload)) && !extractBVID(String(payload))) {
        if (!(await isAllowedFilePath(String(payload)))) return `视频路径不在白名单目录中: ${payload}`

...
示例:
- 图片: send({ type: "image", payload: "url1" })，注: 图片支持一次发送多张，url/文件路径用英文逗号分隔，最多 10 张， send({ type: "image", payload: "url1,url2,url3" })
...
- 视频: send({ type: "video", payload: "BV1xx411c7mD" })`,
  ...
  parameters: {
    ...
    properties: {
      ...
      payload: {
        type: "string",
        description: "发送内容: 图片/文件/语音/视频传 URL 或本地路径；音乐传网易云 ID (纯数字)；视频也可传 BVID (BV开头)",
      },
    },

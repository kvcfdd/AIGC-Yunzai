...
import { mediaToFile, imageFileToDataUris, MAX_IMAGE_BYTES } from "../provider/media.js"
...
async function urlToImageData(url, signal) {
  ...
  try {
    ...
    return { images: [await imageFileToDataUris(file, signal)], paths: [file] }
  } finally {
...
tools.register({
  name: "run_cmd",
  description: `插件功能触发器 — 传入指令文本 (如 #今日运势、#绑定xxx) 以用户身份触发 bot 框架插件的功能。

使用规范:
- 指令写法先调用 skill 工具查看对应插件的 SKILL.md 文档,不要凭空猜测指令
- 不支持多轮/上下文类指令 (如需要连续对话的功能)
- 同类指令不要重复调用 (已成功触发并返回结果后,用户未明确要求不要再触发一次)

示例:
- 查询今日运势: run_cmd({ cmd: "#今日运势" })
- 查询绑定信息: run_cmd({ cmd: "#绑定" })`,
  parameters: {
    type: "object",
    properties: {
      cmd: {
        type: "string",
        description: "指令文本。不确定写法时先调用 skill 工具查看对应插件的 SKILL.md。",
      },
    },
    required: ["cmd"],
  },
  execute: async (args, ctx) => {
    ...
    try {
      ...
      if (texts.length) {
        log.warn(`run_cmd <${cmd.trim()}> 超时但已有返回，功能仍在后台执行`)
        const text = texts.join("\n")
        return images.length ? { images, image_paths, text } : text
      }

...
/** 是否仅主人可使用定时任务子系统 */
function scheduledTaskMasterOnly() {
  return cfg.aigc?.scheduled_task?.master_only ?? true
}
  ...
  return new Promise(resolve => {
    execFile("node", [scriptFile], { timeout: SCRIPT_TIMEOUT_S * 1000, maxBuffer: 64 * 1024 }, (err, stdout, stderr) => {
      if (err) {
        ...
        Bot.makeLog("warn", `[ScheduleTask] 脚本执行异常: ${key}, ${msg.slice(0, 200)}`)
...
async function restoreTasks() {
  ...
  if (!scheduledTaskEnabled()) {
    Bot.makeLog("info", "[ScheduleTask] 定时任务子系统未开启，跳过恢复")
    return
  }
...
可选 script (仅主人): node 检测脚本。触发时先执行脚本，脚本 stdout 非空才将其内容注入对话唤醒 LLM，适合监控类任务 (脚本负责查价/比较，命中条件才输出，未命中静默零成本)。`,
  ...
  execute: async (args, ctx) => {
    ...
    // 总开关: 关闭时拦截所有操作
    if (!scheduledTaskEnabled()) {
      return "定时任务子系统未开启！请在 config/config/aigc.yaml 中设置 scheduled_task.enable: true 后重启再试。"
    }

    // 开启时仅主人可调用本工具
    if (scheduledTaskMasterOnly() && !isMaster) {

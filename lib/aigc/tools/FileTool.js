...
tools.register({
  name: "file",
  description: `统一文件收发工具 — 将图片、文件、语音、音乐卡片或视频下载到本地或发送到当前对话。

提示: 图片/视频/音乐的 URL 或 ID 可先通过 search 工具获取 (type: "image" | "video" | "music")。

参数:
- action: receive | send (receive=下载到本地并返回本地路径, send=发送到当前对话)
- type: image | file | record | music | video
- payload: 图片/文件/语音/视频传 URL 或本地文件路径；音乐传网易云 ID (纯数字)；视频也可传 BVID (BV开头)

示例:
- 接收任意文件: file({ action: "receive", payload: "https://example.com/data.json" })
- 发送图片: file({ action: "send", type: "image", payload: "url1" }), 注: 图片支持一次发送多张, url/文件路径用英文逗号分隔, 最多 10 张，file({ action: "send", type: "image", payload: "url1,url2,url3" })
- 发送文件: file({ action: "send", type: "file", payload: "data/aigc/agent/task_xxx/report.md" })
- 发送语音: file({ action: "send", type: "record", payload: "https://example.com/audio.mp3" })
- 发送音乐卡片: file({ action: "send", type: "music", payload: "56760528" }), 注: 音乐卡片仅支持发送
- 发送视频: file({ action: "send", type: "video", payload: "BV1xx411c7mD" })`,
  ...
  parameters: {
    ...
    properties: {
      action: {
        type: "string",
        enum: ["send", "receive"],
        description: "操作: send=发送, receive=接收",
      },
      type: {
        type: "string",
        enum: ["image", "file", "record", "music", "video"],
        description: "发送类型(send必传): image=图片, file=文件, record=语音, music=音乐卡片, video=视频",
      },
      payload: {
        type: "string",
        description: "receive传 URL ;send传 URL/本地路径/或 ID (音乐/视频)",
      },
    },
  ...
  execute: async (args, ctx) => {
    const e = ctx?.event
    const { action = "send", type, payload } = args
    const raw = String(payload || "").trim()

    if (action === "receive") {
      if (!raw) return "未提供下载地址 (payload)"
      // 本地路径 → 校验存在后直接返回
      if (!isURL(raw)) {
        const resolved = path.resolve(raw)
        try {
          await fsp.access(resolved)
          return `文件路径: ${resolved}`
        } catch {
      ...
      try {
        const local = await downloadToTemp(raw, extFromUrl(raw, ".bin"))
        return `已下载到本地: ${local}`
      } catch (err) {
    ...
    if (type === "image") {
      ...
      for (let i = 0; i < items.length; i += 3) {
        const batch = items.slice(i, i + 3).map(async item => {
          ...
          try {
            await fsp.access(path.resolve(item))
            return item
          } catch {
            return null
          }
    ...
    if (type === "file") {
      if (!raw) return "未提供文件内容/路径/URL"
      let local
      if (isURL(raw)) {
        try {
          local = await downloadToTemp(raw, extFromUrl(raw, ".bin"))
        } catch (err) {
        ...
        try {
          ...
          local = path.join(tmpDir, `text_${crypto.randomUUID().slice(0, 8)}.txt`)
          await fsp.writeFile(local, raw, "utf-8")
        }
      }

      try {
        const filename = path.basename(local)
        await e.reply(segment.file(local, filename))
        const size = ((await fsp.stat(local)).size / 1024).toFixed(1)
    ...
    if (type === "music") {
      const musicId = extractMusicID(raw)
      if (!musicId) return `无法解析音乐 ID: "${raw}"。请提供网易云歌曲 ID (纯数字) 或 music.163.com 链接。`
    ...
    if (type === "video") {
      ...
      if (bvid) {
        const tempDir = path.resolve("data/aigc/tmp/videos", bvid)
        const videoPath = path.join(tempDir, "video.m4s")
        const audioPath = path.join(tempDir, "audio.m4s")
        const outputPath = path.resolve(`data/aigc/tmp/videos/${bvid}.mp4`)

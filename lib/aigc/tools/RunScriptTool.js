...
- fetch(url, options?) — HTTP 请求 (禁止内网)，返回 { status, headers, body }
...
\`\`\``,
  ...
  parameters: {
    ...
    properties: {
      code: {
        type: "string",
        description: `要执行的 JavaScript 代码 (async 函数体)。可使用 callTool/fetch/console.log。用 return 返回结果。`,
      },
      reason: {
        type: "string",
  ...
  execute: async args => {
    ...
    Bot.makeLog("info", `[RunScript] 执行代码${reason ? ` (${reason})` : ""}: ${codePreview}`)
    ...
    try {
      ...
      const timeout = new Promise((_, reject) => setTimeout(() => reject(Object.assign(new Error(`脚本执行超时 (${SCRIPT_TIMEOUT_S}秒)`), { code: "ERR_SCRIPT_EXECUTION_TIMEOUT" })), SCRIPT_TIMEOUT_S * 1000))

...
- 本轮对话中的图片/视频/语音/文件已自动随任务转发给 Agent，无需在 goal 中重复描述；对话历史中的 [xx](路径) 媒体需要由 Agent 用 media 工具自行查看。
...
- clarify_reply: 解答 Agent 询问的问题。
  例: agent({ action: "clarify_reply", task_id: "01J...", answer: "查询最近7天" })
    ...
    execute: async (args, ctx) => {
      ...
      if (action === "clarify_reply") {
        ...
        try {
          ...
          return `回复失败: ${err.message}。请使用任务提交时返回的完整 task_id（ulid 26 位，不是消息标题里截断的短 ID），可通过 agent({action:"check"}) 查询当前任务列表获取。`
        }
      }

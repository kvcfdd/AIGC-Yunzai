...
tools.register(
  {
    name: "agent",
    description: `派发任务给 Agent 后台执行。Agent 是独立的本地工作子模型，运行在 bot 目录下，可以在后台完成多步复杂任务，完成后会告知你。
...
- 它可以在本地运行 shell 命令、读写文件、联网搜索、创建/运行脚本等操作。
...
- check: 查询任务状态。
  例: agent({ action: "check", task_id: "123456789" })

- cancel: 取消运行中的任务。
  例: agent({ action: "cancel", task_id: "123456789" })`,

...
tools.register({
  ...
  execute: async (args, ctx) => {
    ...
    try {
      ...
      // HTML 模式下检测单个窄容器，按比例放大填满内容区
      let scale = 1
      const target = page.locator("#render-target")
      if (isRawHtml && (await target.locator(":scope > *").count()) === 1) {
        const childBox = await target.locator(":scope > *").first().boundingBox()
        if (childBox && childBox.width > 0 && childBox.width < 944) {
          scale = Math.min(944 / childBox.width, 4)
          await target
            .locator(":scope > *")
            .first()
            .evaluate((el, s) => {

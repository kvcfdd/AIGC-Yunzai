...
const RENDER_DIR = "data/aigc/render"
...
tools.register({
  name: "render",
  description: `将 Markdown/HTML 渲染为精美图片 (1080px 宽 PNG)，保存到本地并返回文件路径。适合:
- 需要排版格式的结果 (表格、代码高亮、分级标题)
- 长文本回复会刷屏时
- 搜索结果汇总、数据报告

提示: 可使用 send_media 工具发送或 fetch_media 工具查看。
...
- 渲染 HTML 表格: render({ content: "<table><tr><th>名称</th><th>数值</th></tr>...</table>" })`,
  ...
  execute: async (args, ctx) => {
    if (!ctx?.event) return "Cannot get user context"
    ...
    return browser.runTask(async () => {
      ...
      try {
        ...
        const dir = path.resolve(RENDER_DIR)
        await mkdir(dir, { recursive: true })
        const name = `render_${Date.now()}_${Math.random().toString(36).slice(2, 8)}.png`
        await writeFile(path.join(dir, name), imgBuf)
        return `渲染成功，文件路径: ${RENDER_DIR}/${name}`
      } catch (err) {

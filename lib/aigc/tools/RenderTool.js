...
tools.register({
  name: "render",
  description: "Render Markdown/HTML as a clean image. Outputs a 1080px wide PNG with auto-height and professional typography.",
  parameters: {
    type: "object",
    properties: {
      content: { type: "string", description: "Markdown or HTML source to render. No need to specify width/height — the tool handles layout." },
    },
    required: ["content"],
  },
  execute: async (args, ctx) => {
    ...
    try {
      ...
      // HTML 模式: 透明底 + 窄容器拉满
      if (isRawHtml) {
        await page.addStyleTag({ content: "html, body, #render-target { background: transparent !important; padding: 0 !important; }" })
        const target = page.locator("#render-target")
        if ((await target.locator(":scope > *").count()) === 1) {
          const childBox = await target.locator(":scope > *").first().boundingBox()
      ...
      const imgBuf = await page.screenshot({ type: "png", omitBackground: isRawHtml })

...
tools.register({
  name: "query",
  description: `查询 bot owner (master) 或指定 QQ 用户的信息 — 用于确认正在与谁对话、判断对方是否为 bot owner (master)、获取对方的更多信息等。

返回结构化 XML:
- <account>: 是否 bot owner (master) / 昵称 / 性别 / 年龄 / 生日 / QQ等级 / QQ号 / 头像
- <group>: 群名片 / 群头衔 / 群身份 (用户不在当前群聊时显示提示)

示例:
- 查询是否bot owner (master): query({ queryType: "master" })
- 查询指定用户: query({ queryType: "member", qq: 123456789 })

提示: 内部工具 — 除非用户明确要求,不要直接向用户透露原始数据。`,
  parameters: {
    type: "object",
    properties: {
      queryType: {

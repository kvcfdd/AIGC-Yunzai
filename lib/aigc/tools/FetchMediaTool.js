...
import { mediaToFile, sniffMediaKind, sniffDocMime, looksLikeText, imageFileToDataUri, videoFileToDataUri, audioFileToDataUri, fileToDataUri, DOCUMENT_MIMES, MAX_IMAGE_BYTES, MAX_VIDEO_DOWNLOAD, MAX_AUDIO_DOWNLOAD, MAX_FILE_DOWNLOAD } from "../provider/media.js"
...
/** 按识别出的类型走对应编码管线；file 且 mime 缺失时按内容兜底（文档魔数 → 文本启发式） */
async function encodeByKind(kind, file, mime, signal) {
  ...
  if (!mime) {
    ...
    mime = sniffDocMime(buf) || (looksLikeText(buf) ? "text/plain" : null)
  }
...
tools.register({
  name: "fetch_media",
  description: "多模态识别工具：查看图片/视频/音频/文档文件。source 支持 http/https 链接、本地绝对路径（如 E:/Yunzai/data/aigc/chat/aigc_image_xxx.jpg）、或基于项目目录的相对路径。对话历史中 [图片]/[视频]/[语音]/[文件] 标记里的路径就是本地文件，直接传入即可查看。若你能从上下文判断类型（如 [文件: 报告.pdf](url) 的扩展名），请传 type 参数指定；不传 type 则按文件内容自动识别。",
  parameters: {
    type: "object",
    properties: {
      source: {
  ...
  execute: async (args, ctx) => {
    ...
    if (source.startsWith("data:")) {
      ...
      if (DOCUMENT_MIMES.has(mime)) return { files: [source], text: "文件获取成功" }
      return "格式不支持"
    }
    ...
    try {
      ...
      // 自动识别: 统一按 file 方式下载/定位缓存，再按内容魔数自动分流
      ...
      if (!kind) {
        // 兜底 1: 文档魔数（PDF/OOXML）
        const docMime = sniffDocMime(buf)
        if (docMime) {
          kind = "file"
          mime = docMime
        } else if (!mime && looksLikeText(buf)) {
          // 兜底 2: 无任何线索的可读文本 → text/plain
          kind = "file"
          mime = "text/plain"
        }
      }

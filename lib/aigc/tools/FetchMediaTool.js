...
import { mediaToFile, sniffMediaKind, imageFileToDataUris, videoFileToDataUri, audioFileToDataUri, fileToDataUri, MAX_IMAGE_BYTES, MAX_VIDEO_DOWNLOAD, MAX_AUDIO_DOWNLOAD, MAX_FILE_DOWNLOAD } from "../provider/media.js"
...
async function encodeByKind(kind, file, mime, signal) {
  if (kind === "image") {
    const dataUris = await imageFileToDataUris(file, signal)
    return { images: [dataUris], image_paths: [file], text: "图片获取成功" }
  }
  if (kind === "video") {
    const dataUri = await videoFileToDataUri(file, signal)

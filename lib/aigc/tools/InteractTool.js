...
tools.register({
  name: "interact",
  description: `发送轻互动 — 点赞、戳一戳或发送柴郡猫表情包。想让气氛更友好、活跃气氛、回应对方情绪时使用。

示例:
- 给当前发言用户点赞: interact({ action: "like" })
- 戳一戳指定用户: interact({ action: "poke", target_qq: 123456789 })
- 发送柴郡猫表情包: interact({ action: "sticker" })

提示: 互动是轻量操作,不要频繁连续使用,避免打扰用户。`,
  parameters: {
    type: "object",
    properties: {
      action: {

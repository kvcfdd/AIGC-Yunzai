...
- recall: 撤回群消息（撤回自己的消息不需要权限，但限制发送消息2分钟内可撤回）。`,
  ...
  parameters: {
    ...
    properties: {
      ...
      user: {
        type: "array",
        items: { type: "number" },
        description: "目标用户 QQ，可传单个号码或数组 (kick/mute/set_card 可用)，如 123456 或 [123456, 654321]",
      },
      duration: {
        type: "number",
  ...
  execute: async (args, ctx) => {
    ...
    // 验证 bot 权限
    try {
      const botMemberObj = Bot.pickMember(gid, e.self_id)
      if (!botMemberObj?.getInfo) return `无法获取 Bot 在群 ${gid} 的成员信息`
      const botMember = await botMemberObj.getInfo()

...
tools.register({
  ...
  parameters: {
    ...
    properties: {
      ...
      user_id: {
        type: "number",
        description: "Target user QQ. Required for: kick, ban, unban, set_card, set_admin, set_title",
      },
      user_ids: {
        type: "array",
        items: { type: "number" },
  ...
  execute: async (args, ctx) => {
    ...
    try {
      ...
      if (OWNER_ACTIONS.includes(action)) {
        ...
        if (botMember.role !== "admin" && botMember.role !== "owner") return `Bot is not an admin in group ${gid}`
      }
    } catch {

...
import { MAX_AUDIO_DOWNLOAD } from "../provider/media.js"
...
tools.register({
  name: "send_media",
  description: `统一发送工具 — 将图片、文件、语音、音乐卡片或视频发送到当前对话。

提示: 图片/视频/音乐的 URL 或 ID 可先通过 search 工具获取 (type: "image" | "video" | "music")，图片会自动处理来自pixiv的防盗链。
...
示例:
- 发送图片: send_media({ type: "image", payload: "url1" }), 注: 图片支持一次发送多张, url/文件路径用英文逗号分隔, 最多 10 张，send_media({ type: "image", payload: "url1,url2,url3" })
- 发送文件: send_media({ type: "file", payload: "data/aigc/agent/task_xxx/report.md" })
- 发送语音: send_media({ type: "record", payload: "https://example.com/audio.mp3" })
- 发送音乐卡片: send_media({ type: "music", payload: "56760528" })
- 发送视频: send_media({ type: "video", payload: "BV1xx411c7mD" })`,
  ...
  parameters: {
    ...
    properties: {
      ...
      payload: {
        type: "string",
        description: "URL/本地路径/或 ID (音乐/视频)",
      },
    },
  ...
  execute: async (args, ctx) => {
    ...
    if (type === "file") {
      ...
      try {
        ...
        try {
          const chat = e.isGroup ? `g:${e.group_id}` : `p:${e.user_id}`
          await redis.set(`aigc:file_sent:${e.self_id}:${chat}:${filename}`, "1", { EX: 86400 })
        } catch {}
    ...
    if (type === "record") {
      ...
      if (isURL(raw)) {
        try {
          const local = await downloadToTemp(raw, extFromUrl(raw, ".mp3"), 60000, MAX_AUDIO_DOWNLOAD)

  ...
  parameters: {
    ...
    properties: {
      action: {
        type: "string",
        enum: ["save", "delete"],
        description: "'save' to remember a new fact or update/overwrite an existing one. 'delete' to permanently forget a fact.",
      },
      key: {
        type: "string",
        description: "A short, concise Unicode label (Chinese or English, e.g., '姓名', 'hobby', 'pet_name') summarizing the memory topic. Use snake_case or simple words. Try to reuse existing keys when updating similar topics.",
      },
      value: {
        type: "string",
        description: "The concrete fact to remember (max 100 chars). Required only for 'save'. Write it as a objective, declarative, third-person sentence (e.g., 'The user is a frontend developer' or '用户有一只叫猫猫的橘猫'). Avoid pronouns like 'I', 'You', 'me', 'my' or 'your'.",
      },
    },
    required: ["action", "key"],
  },

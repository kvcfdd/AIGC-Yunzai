...
tools.register({
  name: "task",
  description: `派发后台任务给 Agent 异步执行，适合复杂多步任务。

什么时候用 Agent:
- 需要写代码循环/批处理（Agent 有 sandbox）
- 需要多轮搜索+浏览+汇总
- 任务耗时很长，不想让用户等

什么时候直接用工具:
- 搜一张图/一首歌直接发 → search + send 即可
- 打开一个网页看看 → browse 即可
- 查用户信息 → query 即可
...
每用户最多 ${MAX_USER_TASKS} 个并发。`,
  ...
  execute: async (args, ctx) => {
    ...
    if (action === "check") {
      ...
      const r = await Bot.aigc.agent.check(task_id)
    ...
    if (action === "cancel") {
      if (!task_id) return "请提供任务 ID (task_id)"
      const success = await Bot.aigc.agent.cancel(task_id)
    ...
    if (action === "clarify_reply") {
      ...
      try {
        await Bot.aigc.agent.resume(task_id, answer)

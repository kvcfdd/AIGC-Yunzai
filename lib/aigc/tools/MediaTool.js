...
import {
  ...
  isToolInstalled,
  downloadWithAria2c,
  downloadWithNativeFetch,
  mergeVideoAndAudio,
} from "../helpers/bilibili.js"
...
tools.register({
  name: "send_media",
  description:
    "Send a music card or video to the chat. First search(type='music'|'video') to find content, then use the ID from the results here.",
  parameters: {
    type: "object",
    properties: {
      type: {
        type: "string",
        enum: ["music", "video"],
        description: "Media type — music or video only, NOT image",
      },
      id: {
        type: "string",
        description: "Music ID (Netease) or video BVID (Bilibili) from search results",
      },
    },
    required: ["type", "id"],
  },

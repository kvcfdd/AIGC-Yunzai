...
tools.register({
  name: "send_image",
  description:
    "Download images and send them to the chat. First search(type='image') to find images, then pass the results here. DO NOT make up URLs.",
  parameters: {
    type: "object",
    properties: {
      images: {
        ...
        items: {
          ...
          properties: {
            url: {
              type: "string",
              description: "Image URL from search results — DO NOT make up",
            },
            referer: {
              type: "string",
              description: "Referer from search results, required for most image sources",
            },
          },
          required: ["url"],
        },
      ...
      useProxy: {
        type: "boolean",
        description: "Whether to use proxy for downloading. Default: false (direct connection)",
      },
    },
    required: ["images"],
  },
  execute: async (args, ctx) => {
    ...
    if (!localPaths.length)
      return "All images failed to download — reply in text instead of retrying"

...
tools.register({
  name: "search",
  description:
    "Search the internet for web pages, images, music, or videos. Use when you need real-time info or want to find media to share. After searching the web, browse specific results for more details.",
  parameters: {
    type: "object",
    properties: {
    ...
    if (type === "music") {
      try {
        const res = await fetch(
          `http://music.163.com/api/search/get/web?s=${encodeURIComponent(q)}&type=1&offset=0&total=true&limit=${limit}`,
        )
    ...
    if (type === "video") {
      try {
        ...
        const resp = await fetch(
          `https://api.bilibili.com/x/web-interface/wbi/search/type?${apiParams.toString()}`,
          { headers },
        )

...
搜索结果的使用方式:
- 搜网页: 可用 browse 打开具体结果页面获取全文(数据源来自Bing，baidu，duckduckgo)
- 搜图片: 取结果的 URL，用 send_media({ type:"image" }) 发送 (数据源来自Bing和pixiv)
- 搜音乐: 取结果的网易云 ID，用 send_media({ type:"music" }) 发送
- 搜视频: 取结果的 BVID，用 send_media({ type:"video" }) 发送`,
  parameters: {
    type: "object",
    properties: {

...
import { mediaToFile, imageFileToDataUri, videoFileToDataUri, audioFileToDataUri, fileToDataUri, isAnimatedGif, MAX_IMAGE_BYTES, MAX_VIDEO_DOWNLOAD, MAX_AUDIO_DOWNLOAD, MAX_FILE_DOWNLOAD } from "./media.js"
...
class LlmProvider {
    ...
    await Promise.all(
      urls.map(async (u, i) => {
        ...
        try {
          ...
          // 动画 GIF
          if (file.toLowerCase().endsWith(".gif") && (await isAnimatedGif(file)) === true) {
            uris[i] = await videoFileToDataUri(file, false, undefined, { gif: true })
          } else {
  ...
  /** 接收文件 → 缓存原文件 → 格式识别 → data URI，无法识别为 [文件异常]
  ...
  async resolveFiles(segs, signal) {
    ...
    for (const seg of segs) {
      ...
      try {
        ...
        if (!mime) {
          log.debug(`文件 ${seg?.name || file.split("/").pop()} 格式无法识别`)
          uris.push("[文件异常]")
          paths.push(null)
          continue
        }

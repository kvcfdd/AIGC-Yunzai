...
import { OFFICE_MIMES, convertOfficeDoc } from "./office.js"
import { mediaToFile, imageFileToDataUris, videoFileToDataUri, audioFileToDataUri, fileToDataUri, MAX_IMAGE_BYTES, MAX_VIDEO_DOWNLOAD, MAX_AUDIO_DOWNLOAD, MAX_FILE_DOWNLOAD } from "./media.js"
 ...
 *
 *  chat() 契约:
 *   options: { signal, tools, tool_choice, channel, model, ...适配器透传 }
 *   返回:    { content, tool_calls?, reasoning_content?, reasoning_parts?, blocked?, finishReason?, usage? }
...
class LlmProvider {
   ...
   *  超长图在此切成多段
   *  uris 对应位置为字符串数组
   *  paths 始终每张源图一项，供落盘时以 [xx](路径) 引用 —— 不可随切段展开
   *  否则 engine 侧占位符与路径的对位会错乱
   *  @returns {Promise<{ uris: (string|string[])[], paths: (string|null)[] }|null>} */
    ...
    await Promise.all(
      urls.map(async (u, i) => {
        ...
        try {
          ...
          uris[i] = await imageFileToDataUris(file, signal)
          paths[i] = file
        } catch (err) {
  ...
  async resolveVideo(urls, signal) {
    if (!urls?.length) return null
    const uris = []
    const paths = []
    for (const u of urls) {
      try {
        ...
        uris.push(await videoFileToDataUri(file, signal))
        paths.push(file)
      } catch (err) {
   ...
   *  与用户直接发图/视频走同一管线
   *  OOXML 工作文档先本地预处理为白名单文本格式再发送，file_paths 仍记原件；
   *  转换失败降级为不支持提示
    ...
    for (const seg of segs) {
      ...
      try {
        ...
        if (OFFICE_MIMES.has(mime)) {
          const conv = await convertOfficeDoc(file, mime)
          if (!conv) {
            files.push("[文件格式不支持查看]")
            file_paths.push(null)
            continue
          }
          files.push(`data:${conv.mime};base64,${Buffer.from(conv.text, "utf8").toString("base64")}`)

...
async function geminiChat(messages, options = {}) {
  ...
  const body = {
    ...
    input,
    generation_config: {
      max_output_tokens: options.max_tokens ?? cfg.aigc?.max_tokens ?? 8089,
    },
  }
  ...
  if (isGeminiModel(model)) {
    ...
    body.generation_config.thinking_summaries = cfg.aigc?.show_thinking ? "auto" : "none"
  }
    ...
    for (let tryCount = 0; tryCount < keyTries; tryCount++) {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          throw new AigcError(String(res.status), `API 错误 [${res.status}]: ${text}`)
        }

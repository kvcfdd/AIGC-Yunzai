...
const MAX_VIDEO_BYTES = 50 * 1024 * 1024 // 压缩后输出上限 50MB（base64 约 67MB，低于 100MB 请求体上限）
export const MAX_VIDEO_DOWNLOAD = 80 * 1024 * 1024 // 下载上限 80MB
const MAX_VIDEO_PX = 720 // 抽帧后目标高度
const VIDEO_CRF = 20 // 画质优先
const VIDEO_FAST_THRESHOLD = 5 * 1024 * 1024 // 低于此值跳过压缩, 仅抽帧
const VIDEO_FAST_CRF = 18 // 小视频仅抽帧时的视觉无损档位
export const MAX_AUDIO_DOWNLOAD = 25 * 1024 * 1024 // 下载上限 25MB/条（QQ 语音 ≤60s 的 silk 通常 <1MB）
export const MAX_AUDIO_BYTES = 50 * 1024 * 1024 // FLAC 输出上限 50MB
...
// Gemini document 部件实际支持内联的 MIME。
// 识别出但不在列内的格式: 可读文本降级为 text/plain，二进制视为无法识别
const GEMINI_DOCUMENT_MIMES = new Set([
  "application/pdf",
  "text/plain",
  "text/markdown",
  "text/html",
...
async function imageFileToDataUri(file) {
  ...
  const quality = (await stat(file)).size < IMAGE_FAST_THRESHOLD ? 100 : WEBP_QUALITY
...
async function videoFileToDataUri(file, removeAudio = false, signal) {
  ...
  try {
    ...
    const smallVideo = inSize < VIDEO_FAST_THRESHOLD
    const vf = smallVideo ? `fps=${probeFps}` : `fps=${probeFps},scale='min(1280,iw)':'min(${MAX_VIDEO_PX},ih)':force_original_aspect_ratio=decrease`
    const crf = smallVideo ? VIDEO_FAST_CRF : VIDEO_CRF

    // 抽帧 + 压缩
    await new Promise((resolve, reject) => {
      const ff = exec(`ffmpeg -i "${file}" -vf "${vf}" -c:v libx264 -crf ${crf} -preset medium -pix_fmt yuv420p -movflags +faststart${audioFlag} -y "${outFile}"`, { timeout: 60000 }, err => (err ? reject(err) : resolve()))
      ff.stderr?.on("data", () => {})
      if (signal) {
        onAbort = () => {
          ff.kill()
...
async function audioFileToDataUri(file, signal) {
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      const ff = exec(`ffmpeg -i "${file}" -vn -acodec flac -ar ${SILK_SAMPLE_RATE} -ac 1 -y "${outFile}"`, { timeout: 60000 }, err => (err ? reject(err) : resolve()))
      ff.stderr?.on("data", () => {})
      if (signal) {
        onAbort = () => {
          ff.kill()
...
export { GEMINI_DOCUMENT_MIMES, mediaToFile, sniffMediaKind, imageFileToDataUri, videoFileToDataUri, audioFileToDataUri, fileToDataUri }

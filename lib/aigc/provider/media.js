...
async function mediaToFile(source, { kind = "image", headers = {}, dispatcher, maxBytes, signal, name } = {}) {
  ...
  if (typeof source === "string" && source.startsWith("data:")) {
    ...
    const docMime = GEMINI_DOCUMENT_MIMES.has(mime) || OFFICE_MIMES.has(mime) ? mime : looksLikeText(buf) ? "text/plain" : null
 ...
 *  Gemini 不支持的格式: OOXML 工作文档可本地预处理，放行交由转换层；
...
function resolveDocMime(buf, ext) {
  ...
  if (!mime || GEMINI_DOCUMENT_MIMES.has(mime) || OFFICE_MIMES.has(mime)) return mime

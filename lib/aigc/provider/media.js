...
import { OFFICE_MIMES } from "./office.js"

const MAX_IMAGE_PX = 1536
const IMAGE_FAST_THRESHOLD = 1.5 * 1024 * 1024 // 低于此值压缩无意义, 100 质量统一格式
const IMAGE_SLICE_TRIGGER_H = MAX_IMAGE_PX * 1.5 // 超过此高度才切
const IMAGE_SLICE_OVERLAP = 20 // 相邻段重叠像素，避免拦腰截断文字行
const MAX_IMAGE_SLICES = 20 // 单图段数上限
const SLICE_BUDGET_H = MAX_IMAGE_SLICES * MAX_IMAGE_PX - IMAGE_SLICE_OVERLAP * (MAX_IMAGE_SLICES - 1) // 可切的最大高度: MAX_IMAGE_SLICES 段 x 段高上限, 减去相邻段的重叠部分
...
const DOCUMENT_MIMES = new Set(["application/pdf", "text/plain", "text/markdown", "text/html", "text/css", "text/javascript", "text/x-typescript", "text/x-python", "text/xml", "text/csv", "text/rtf", "application/json"])
 ...
 *  括号也在剔除之列: 缓存路径会被写进 [图片](路径) 形式的文本标记，
 *  而标记以 ) 收尾 —— 路径里带括号就会把解析截断 */
...
async function mediaToFile(source, { kind = "image", headers = {}, dispatcher, maxBytes, signal, name } = {}) {
  ...
  if (typeof source === "string" && source.startsWith("data:")) {
    ...
    const docMime = DOCUMENT_MIMES.has(mime) || OFFICE_MIMES.has(mime) ? mime : looksLikeText(buf) ? "text/plain" : null
    const mediaKind = docMime ? null : sniffMediaKind(buf, null)
    if (!docMime && !mediaKind) {
      log.debug(`文件格式无法识别，不缓存: ${name || "(data URI)"}`)
      return { file: null, mime: null }
    }
    const saveExt = mediaKind ? sniffExt(buf, mediaKind) : fileSaveExt(mimeToExt(mime), docMime)
    return { file: await saveToCache(buf, kind, saveExt, safeFileBase(name)), mime: docMime }
  }
  if (/^https?:\/\//i.test(source)) {
    ...
    const mediaKind = mime ? null : sniffMediaKind(buf, null)
    ...
    const saveExt = mediaKind ? sniffExt(buf, mediaKind) : fileSaveExt(ext, mime)
    return { file: await saveToCache(buf, kind, saveExt, safeFileBase(rawName || name)), mime }
  }
  ...
  const saveExt = mediaKind ? sniffExt(buf, mediaKind) : fileSaveExt(ext, mime)
  return { file: await saveToCache(buf, kind, saveExt, safeFileBase(name || path.basename(abs))), mime }
}
...
/** 纵向切段方案: 段高均匀, 故先按「含重叠可覆盖全高」求最少段数, 再均分
 *  @param {number} h 图高
 *  @returns {Array<{ top: number, height: number }>|null} null 表示段数超上限, 调用方应降级 */
function planImageSlices(h) {
  const slices = Math.ceil((h - IMAGE_SLICE_OVERLAP) / (MAX_IMAGE_PX - IMAGE_SLICE_OVERLAP))
  if (slices <= 1 || slices > MAX_IMAGE_SLICES) return null

  const segH = Math.ceil((h + (slices - 1) * IMAGE_SLICE_OVERLAP) / slices)
  const step = segH - IMAGE_SLICE_OVERLAP
  const plan = []
  for (let k = 0; k < slices; k++) {
    const top = k * step
    plan.push({ top, height: Math.min(segH, h - top) })
  }
  return plan
}

/** 缓存图片文件 → data URI 数组；超长图切为若干等高段，其余情况返回单元素
 *  动画 GIF 特化转视频编码。所有送模型的图片都应经此函数 —— 直接调
 *  imageFileToDataUri 会走 fit-inside，把长图的宽度压扁
 *  @returns {Promise<string[]>} */
async function imageFileToDataUris(file, signal) {
  // 动画 GIF 走视频管线，不做切分
  if (typeof file === "string" && file.toLowerCase().endsWith(".gif") && (await isAnimatedGif(file)) === true) {
    return [await videoFileToDataUri(file, signal, { gif: true })]
  }
  ...
  try {
    ...
    log.debug(`图片元数据读取失败，按单张处理: ${err.message}`)
    return [await imageFileToDataUri(file)]
  }
  if (!meta.width || !meta.height) return [await imageFileToDataUri(file)]

  // 先按宽度封顶，否则超宽图切完每段依然过宽
  let source = file
  let width = meta.width
  let height = meta.height
  if (width > MAX_IMAGE_PX) {
    source = await sharp(file).resize(MAX_IMAGE_PX, null, { withoutEnlargement: true }).toBuffer()
    const scaled = await sharp(source).metadata()
    width = scaled.width
    height = scaled.height
  }

  if (height <= IMAGE_SLICE_TRIGGER_H) return [await imageFileToDataUri(file)]

  // 超出可切上限时先等比压低到预算内，再按同一规则切 —— 此处不能退回 fit-inside 单张，
  // 那会把 1500x30000 这类图压成几十像素宽的糊线，等于静默丢内容
  if (height > SLICE_BUDGET_H) {
    source = await sharp(source).resize(MAX_IMAGE_PX, SLICE_BUDGET_H, { fit: "inside", withoutEnlargement: true }).toBuffer()
    const shrunk = await sharp(source).metadata()
    width = shrunk.width
    height = shrunk.height
    log.debug(`长图高 ${meta.height} 超可切上限 ${SLICE_BUDGET_H}，已等比压低至 ${width}x${height}`)
  }

  const plan = planImageSlices(height)
  if (!plan) return [await imageFileToDataUri(file)]

  const inputSize = typeof file === "string" ? (await stat(file)).size : file.length
  const quality = inputSize < IMAGE_FAST_THRESHOLD ? 100 : 90
  const slice = (top, segH, fmt) => {
    const image = sharp(source).extract({ left: 0, top, width, height: segH })
    return fmt === "webp" ? image.webp({ quality }).toBuffer() : image.jpeg({ quality }).toBuffer()
  }

  try {
    const parts = []
    for (const { top, height: segH } of plan) {
      // 单段 extract 可能耗时数百毫秒，段间需重新检查中断
      if (signal?.aborted) throw new DOMException("Aborted", "AbortError")
      try {
        parts.push(`data:image/webp;base64,${(await slice(top, segH, "webp")).toString("base64")}`)
      } catch (err) {
        log.debug(`WebP 切段失败，尝试 JPEG: ${err.message}`)
        parts.push(`data:image/jpeg;base64,${(await slice(top, segH, "jpeg")).toString("base64")}`)
      }
    }
    log.debug(`长图 ${meta.width}x${meta.height} 切为 ${parts.length} 段 (段高 ${plan[0].height})`)
    return parts
  } catch (err) {
    if (err?.name === "AbortError") throw err
    log.warn(`长图切分失败，退回单张压缩: ${err.message}`)
    return [await imageFileToDataUri(file)]
  }
}
...
async function videoFileToDataUri(file, signal, opts = {}) {
  ...
  try {
    ...
    try {
      ...
      if (dur > 0) log.debug(`${gif ? "GIF 动画" : "视频时长"} ${dur.toFixed(1)}s → ${probeFps}fps（约 ${Math.ceil(dur)} 帧）`)
    } catch {
      // ffprobe 不可用
    }
    ...
    if (gif) {
      ...
      vf = `fps=${probeFps},scale='min(1920,iw)':'min(1080,ih)':force_original_aspect_ratio=decrease,scale='trunc(iw/2)*2':'trunc(ih/2)*2'`
    }

    // 抽帧 + 压缩
    await new Promise((resolve, reject) => {
      const ff = exec(`ffmpeg -i "${file}" -vf "${vf}" -c:v libx264 -crf 18 -preset medium -pix_fmt yuv420p -r ${probeFps} -movflags +faststart -y "${outFile}"`, { timeout: 30000 }, err => (err ? reject(err) : resolve()))
      ff.stderr?.on("data", () => {})
      if (signal) {
        onAbort = () => {
          ff.kill()
...
function sniffDocMime(buf) {
  ...
  if (buf.length >= 4 && latin1(0, 4) === "PK\x03\x04") {
    const head = latin1(0, Math.min(buf.length, 65536))
 ...
 *  白名单外的二进制无法作为文档下发；OOXML 工作文档可本地预处理后放行，交由转换层；
 *  其余可读文本降级为 text/plain，二进制返回 null */
function resolveDocMime(buf, ext) {
  const mime = sniffDocMime(buf) || EXT_MIME[ext] || (looksLikeText(buf) ? "text/plain" : null)
  if (!mime || DOCUMENT_MIMES.has(mime) || OFFICE_MIMES.has(mime)) return mime
...
export { mediaToFile, sniffMediaKind, imageFileToDataUris, videoFileToDataUri, audioFileToDataUri, fileToDataUri, classifyFileExt }

...
async function buildOpenAIMessages(messages) {
  ...
  for (const msg of messages) {
    ...
    if (msg.time && msg.role === "user") {
      const ctx = msg.chat_type
        ? msg.chat_type === "群聊" && msg.group_name
          ? ` ${msg.chat_type} ${msg.group_name}`
          : ` ${msg.chat_type}`
        : ""
    ...
    if (msg.role === "tool" && msg.images?.length) {
      // OpenAI tool 消息不支持 images → 拆为 tool(text) + user(multimodal)
      apiMsg.content = text
      result.push(apiMsg)
      const imgParts = [{ type: "text", text: "[工具获取的图片]" }]
...
/** 从逗号分隔的模型列表中轮询选取 */
function _pickModel(config, options) {
  const raw = options.model || config.model || ""
  ...
  const idx = _modelIdx % models.length
  _modelIdx++
  return models[idx]
}
...
async function openaiChat(messages, options = {}) {
  ...
  for (let attempt = 0; attempt < rotated.length; attempt++) {
    const model = _pickModel(config, options) || "gpt-4o-mini"
    body.model = model
...
async function geminiChat(messages, options = {}) {
  ...
  const endpoint =
    options.endpoint || config.endpoint || "https://generativelanguage.googleapis.com"
  ...
  const body = {
    ...
    generationConfig: {
      ...
      temperature: options.temperature ?? cfg.aigc?.temperature ?? 0.7,
      thinkingConfig: {
        thinkingLevel: config.thinking_level ?? "low",
  ...
  for (let attempt = 0; attempt < rotated.length; attempt++) {
    const model = _pickModel(config, options) || "gemini-3.5-flash"
    const api_key = rotated[attempt]
    const t0 = Date.now()
    try {
  ...
  for (const msg of messages) {
    ...
    if (msg.role === "user") {
      ...
      if (msg.content) {
        ...
        const text = msg.time
          ? `[${formatMsgTime(msg.time)}${ctx}] ${msg.content}`
          : String(msg.content)
        parts.push({ text })
      }

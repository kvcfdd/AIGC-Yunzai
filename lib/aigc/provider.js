    ...
    if (msg.role === "assistant") {
      ...
      if (msg.reasoning_parts?.length) {
        for (const rp of msg.reasoning_parts) {
          const step = {
            type: "thought",
            summary: [{ type: "text", text: rp.text }],
          }
          if (rp.signature) step.signature = rp.signature
          input.push(step)
        }
      }
      ...
      if (msg.content) {
        const mc = [{ type: "text", text: msg.content }]
        // 携带 text content 的签名（如有），保持 content_parts 位置上下文
        if (msg.content_parts?.length) {
          const sig = msg.content_parts[0]?.signature
          if (sig) mc[0].signature = sig
        }
        input.push({
          type: "model_output",
          content: mc,
...
function parseInteractionSteps(steps, usage) {
  ...
  for (const step of steps) {
      ...
      if (step.content) {
        for (const c of step.content) {
          if (c.type === "text" && c.text) {
            content += c.text
            contentParts.push({ text: c.text, signature: c.signature || null })
          }
        }
      }
    }
    ...
    if (step.type === "function_call") {
      tool_calls.push({
        ...
        ...(step.thought_signature ? { thought_signature: step.thought_signature } : {}),
      })
    }
  }
  ...
  return {
    ...
    content_parts: contentParts.length ? contentParts : undefined,
    usage: usage
      ? {

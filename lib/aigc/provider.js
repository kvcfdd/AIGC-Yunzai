...
async function videoToDataUri(url) {
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      const ff = exec(`ffmpeg -i "${inFile}" -vf "scale='min(1280,iw)':'min(${MAX_VIDEO_PX},ih)':force_original_aspect_ratio=decrease" -an -c:v libx264 -crf 28 -preset fast -movflags +faststart -y "${outFile}"`, { timeout: 60000 }, err => (err ? reject(err) : resolve()))
      ff.stderr?.on("data", () => {})
    })
    ...
    try {
      await new Promise((resolve, reject) => {
        const ff = exec(`ffmpeg -i "${inFile}" -an -c copy -y "${outFile}"`, { timeout: 30000 }, err => (err ? reject(err) : resolve()))
        ff.stderr?.on("data", () => {})
      })
      const stripped = await readFile(outFile)
...
async function openaiChat(messages, options = {}) {
  ...
  const body = {
    ...
    temperature: options.temperature ?? cfg.aigc?.temperature ?? 0.9,
  }
  ...
  for (let attempt = 0; attempt < rotated.length; attempt++) {
    ...
    try {
      ...
      if (attempt < rotated.length - 1) {
        log.warn(`OpenAI API 尝试 ${attempt + 1}/${rotated.length - 1} 失败: ${err.message}，3秒后重试...`)
...
class AigcProvider {
  ...
  async resolveVideo(urls) {
    if (!urls?.length) return null
    const resolved = []
    for (const url of urls) {
      try {
        const uri = await videoToDataUri(url)
        resolved.push(uri)
      } catch (err) {
        log.debug(`视频处理失败: ${err.message}`)
        resolved.push("[视频异常]")

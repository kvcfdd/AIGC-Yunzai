...
function _rotateKeys(keys) {
  const retry = Math.min(Math.max(cfg.aigc?.retry_count ?? 0, 0), 5)
  ...
  for (let attempt = 0; attempt < rotated.length; attempt++) {
    ...
    try {
      ...
      if (attempt < rotated.length - 1) {
        log.warn(`OpenAI API 尝试 ${attempt + 1}/${rotated.length} 失败: ${err.message}，3秒后重试...`)
...
async function geminiChat(messages, options = {}) {
  ...
  for (let attempt = 0; attempt < rotated.length; attempt++) {
    ...
    try {
      ...
      lastError = err
      if (attempt < rotated.length - 1) {
        log.warn(`Gemini API 尝试 ${attempt + 1}/${rotated.length} 失败: ${err.message}，3秒后重试...`)
        await Bot.sleep(3000)
      }
    }
  }

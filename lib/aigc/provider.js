...
async function imageToDataUri(url, headers = {}, dispatcher) {
  ...
  const compress = async fmt => {
    const image = sharp(buffer).resize(MAX_IMAGE_PX, MAX_IMAGE_PX, {
      fit: "inside",
  ...
  try {
    ...
    try {
      const dur = await new Promise((resolve, reject) => {
        const p = exec(`ffprobe -v error -show_entries format=duration -of csv=p=0 "${inFile}"`, { timeout: 10000 }, (err, stdout) => {
    ...
    await new Promise((resolve, reject) => {
      const ff = exec(`ffmpeg -i "${inFile}" -vf "fps=${probeFps},scale='min(1280,iw)':'min(${MAX_VIDEO_PX},ih)':force_original_aspect_ratio=decrease" -c:v libx264 -crf ${VIDEO_CRF} -preset medium -pix_fmt yuv420p -movflags +faststart${audioFlag} -y "${outFile}"`, { timeout: 60000 }, err => (err ? reject(err) : resolve()))
      ff.stderr?.on("data", () => {})
      if (signal) {
        onAbort = () => {
          ff.kill()

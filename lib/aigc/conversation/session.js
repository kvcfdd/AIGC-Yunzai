...
export default {
  ...
  async getMessages(self_id, user_id, maxRounds = 10) {
    ...
    // 最近 maxRounds 轮完整保留；媒体已不落盘编码，无需降级
  ...
  /** 按轮次裁剪到指定轮数: 最近 maxRounds 轮完整保留(含工具调用记录)，更早轮次压缩为纯 QA
   *  媒体落盘已是 [xx](路径) 标记且缓存文件长存，无需降级替换，标记原样保留 */
  trimTo(session, maxRounds = 10) {
    ...
    for (let i = 0; i < totalRounds; i++) {
      if (i >= fullStart) {
        // 最近 maxRounds 轮完整保留(含工具调用记录)
        processed.push(...rounds[i])
      } else {
        // 老轮次压缩：只保留 user 问 + 最终 assistant 答
        // 媒体字段落盘时已删除，此处兜底清理，标记文本不动
        const userMsg = rounds[i].find(m => m.role === "user")
        if (userMsg) {
          const trimmed = { ...userMsg }
          delete trimmed.images
          delete trimmed.image_paths
          delete trimmed.videos
          delete trimmed.video_paths
          delete trimmed.audios
          delete trimmed.audio_paths
          delete trimmed.files
          delete trimmed.file_paths
  ...
  extractQA(messages, opts = {}) {
    ...
    const MAX_TOOL_TEXT = 300 // 单个工具结果展示上限，避免超长结果撑爆总结输入
    const lines = []

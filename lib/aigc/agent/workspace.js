...
class WorkspaceManager {
  ...
  /** 标记交付文件列表 — 记录哪些文件需要主模型发送给用户 */
  async markDeliverFiles(files) {
    const manifest = { files, markedAt: Date.now() }
    await this.writeFile("__deliver__.json", JSON.stringify(manifest, null, 2))
    return manifest
  }

  /** 读取交付文件列表 */
  async getDeliverFiles() {
    try {
      const content = await this.readFile("__deliver__.json")
  ...
  _validatePath(filename) {
    ...
    if (!basename || basename === ".." || basename.includes("/") || basename.includes("\\")) {
      throw new Error("文件名无效，仅允许纯文件名")
    }
    if (basename.length > 120) {
      throw new Error("文件名过长 (最大 120 字符)")
    }
  }
}
...
async function cleanupExpiredWorkspaces() {
  ...
  try {
    ...
    if (cleaned > 0) {
      Bot.makeLog("info", `[Agent] 清理 ${cleaned} 个过期工作区 (TTL: ${ttl}s)`)
    }
  } catch {
    /* pass */
  }

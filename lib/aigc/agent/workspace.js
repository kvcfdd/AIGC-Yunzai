...
/** 全局知识库: 所有 Agent 任务共享，位于项目根目录，bash 工具维护，启动时注入系统提示词 */
const KNOWLEDGE_FILE = path.resolve("knowledge.md")
const KNOWLEDGE_MAX_CHARS = 12000

/** 读取全局知识库并裁剪超限内容 (供 runner 注入系统提示词)，不存在或为空返回 null */
async function loadKnowledge() {
  try {
    let raw = await fs.readFile(KNOWLEDGE_FILE, "utf-8")
    if (!raw.trim()) return null
    // 超限裁剪: 从最旧行开始丢弃，并落盘
    let trimmed = raw
    while (trimmed.length > KNOWLEDGE_MAX_CHARS) {
      const nl = trimmed.indexOf("\n")
      if (nl === -1) break
      trimmed = trimmed.slice(nl + 1)
    }
    if (trimmed !== raw) await fs.writeFile(KNOWLEDGE_FILE, trimmed, "utf-8")
...
async function cleanupExpiredWorkspaces() {
  const ttl = cfg.agent?.workspace_ttl
  if (!ttl || ttl <= 0) return
...
export { WorkspaceManager, cleanupExpiredWorkspaces, AGENT_DIR, loadKnowledge }

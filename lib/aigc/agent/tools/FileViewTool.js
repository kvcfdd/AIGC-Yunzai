...
agentTools.register({
  name: "file_view",
  description: `查看本地文件内容。

- 文本文件: 分页按行返回 (带行号)，offset 起始行、limit 每页行数
- 图片: 直接返回视觉输入
- PDF: 提取文本 (扫描版/加密 PDF 无法提取；需理解版式/表格时用 media 工具)
- 其他二进制: 拒绝，视频/音频用 media 工具`,
  parameters: {
    type: "object",
    properties: {
      ...
      pages: { type: "string", description: '页码或页范围 (仅 PDF)，如 "3"、"1-5"、"1,3,5"，默认全部页' },
    },
    required: ["file"],
  },

  execute: async (args, ctx) => {
    const { file, offset, limit, pages: pagesArg } = args
    ...
    if (head.toString("latin1").startsWith("%PDF-")) {
      ...
      // pages 参数: 指定页或页范围，缺省全部页
      let selectedNums
      let rangeDesc = `全部 ${pages.length} 页`
      if (pagesArg) {
        const idxs = []
        for (const part of String(pagesArg).split(",")) {
          const m = part.trim().match(/^(\d+)\s*-\s*(\d+)$|^(\d+)$/)
          if (!m) return `pages 格式无效: "${pagesArg}"，示例: "3"、"1-5"、"1,3,5"`
          if (m[1]) {
            for (let p = Number(m[1]); p <= Number(m[2]); p++) idxs.push(p - 1)
          } else {
            idxs.push(Number(m[3]) - 1)
          }
        }
        selectedNums = [...new Set(idxs)].filter(i => i >= 0 && i < pages.length).sort((a, b) => a - b)
        if (!selectedNums.length) return `pages 超出范围: PDF 共 ${pages.length} 页 (请求: "${pagesArg}")`
        rangeDesc = selectedNums.length === 1 ? `第 ${selectedNums[0] + 1} 页` : `第 ${selectedNums.map(n => n + 1).join(", ")} 页`
      }
      const total = (selectedNums ? selectedNums.map(i => pages[i]) : pages).join("\n\n")
      const truncated = total.length > MAX_PDF_CHARS ? total.slice(0, MAX_PDF_CHARS) + `\n...(已截断, 共 ${total.length} 字符)` : total
      return `PDF 文件: ${filepath} (共 ${pages.length} 页, ${rangeDesc}, ${(buf.length / 1024).toFixed(1)}KB)\n\n${truncated}`
    }

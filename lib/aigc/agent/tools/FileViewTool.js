...
agentTools.register({
  name: "file_view",
  description: `通用文件查看器 — 自动识别文件类型并返回可读内容。

支持:
- 文本/代码/日志/数据文件: 分页按行返回 (带行号)
- 图片 (png/jpg/gif/webp 等): 转码为视觉输入供模型直接看图
- PDF: 提取文本内容 (扫描版/加密 PDF 无法提取)
- 其他二进制: 明确拒绝
  ...
  execute: async (args, ctx) => {
    ...
    try {
      ...
      if (ft?.mime?.startsWith("image/")) {
        if (buf.length > MAX_IMAGE_BYTES) return `图片过大 (${(buf.length / 1024 / 1024).toFixed(1)}MB)，上限 10MB`
        let dims = ""
        try {

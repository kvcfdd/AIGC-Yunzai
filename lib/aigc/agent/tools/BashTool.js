...
agentTools.register({
  name: "bash",
  description: `执行 shell 命令 — Agent 的核心本地操作工具。可执行任何系统命令(安装/运行脚本、操作文件、调用 CLI 工具、git 等)。

用法建议:
- 默认在任务工作区目录执行；可用 workdir 指定其他目录
- 命令失败会返回 exit code 和 stderr，根据错误信息修正后重试
- 长时间任务(下载、编译、批量处理)设 background: true 后台运行，输出写入日志文件，用 tail/type 查看进度
- 破坏性命令(递归删根目录、格式化磁盘、关机等)会被安全策略自动拦截

当前工作区: 任务专属目录。工作区文件由 workspace 工具管理。`,
  parameters: {
    type: "object",
    properties: {
      ...
      workdir: {
        type: "string",
        description: "执行目录 (绝对路径或相对工作区路径)，默认工作区目录",
      },
      background: {
        type: "boolean",
  ...
  execute: async (args, ctx) => {
    ...
    const baseDir = ctx.workspace?.dir || process.cwd()
    ...
    if (background === true) {
      ...
      const child = spawn(isWin ? "cmd.exe" : "bash", [isWin ? "/c" : "-c", command], {
        cwd,
        detached: true,
    ...
    // 跨平台 shell: Windows 用 cmd.exe (ComSpec)，其余用 sh
    const isWin = process.platform === "win32"
      ...
      exec(
        ...
        (err, stdout, stderr) => {
          ...
          if (err) {
            ...
            // maxBuffer 超限时 err.message 里带完整输出，取尾部
            if (!stdout && err.stdout) stdout = err.stdout
            if (!stderr && err.stderr) stderr = err.stderr
          }
          resolve({ code, timedOut, stdout: decodeOutput(stdout), stderr: decodeOutput(stderr) })
        },
      )
    })

    const out = truncate(result.stdout.trimEnd(), MAX_OUTPUT_CHARS)
    const err = truncate(result.stderr.trimEnd(), MAX_OUTPUT_CHARS)

...
- browse({ url, useProxy? }) — 浏览网页内容，被墙网站设 useProxy: true
- media({ url, type? }) — 下载查看媒体文件内容
...
请只输出可执行的 JavaScript 代码，不要输出解释。`
    ...
    log.info(`[Agent-CodeGen] 生成代码, spec="${spec.slice(0, 100)}${spec.length > 100 ? "..." : ""}"`)
    ...
    log.info(`[Agent-CodeGen] 生成 ${code.length} 字符代码`)
    return code
  },
})

...
const AGENT_SYSTEM = [
  ...
  "4. 文件分工: file_list 列文件 → file_search 定位 → file_view 查看 → file_edit 局部修改 → file_write 新建/整体覆盖项目文件；任务工作区产物用 workspace",
  ...
  "9. 全局避坑指南 knowledge.md(项目根目录): 发现本地跑命令有哪些遇到的坑，怎么解决的，有必要的话可用 bash 写入(没源文件就自己创建)，这是给下一次执行任务的你自己看的，以帮助你自己避坑，保持文件简洁有效，宁缺毋滥！",
...
export class AgentRunner {
  ...
  async run(entry) {
    ...
    const agentModel = cfg.agent?.model || "gemini-3.5-flash"
    if (!agentModel) {
      this.status = "failed"
    ...
    const systemPrompt = [
      ...
      `\n<workspace>\n${this.ws.dir}\n</workspace>\n(workspace 为任务工作区，所有任务产物写入此目录，访问时使用绝对路径)`,
      knowledgeNote,
      skillsNote,
      proxyNote,
    ]

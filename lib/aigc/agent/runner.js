...
// 历史上下文压缩
const KEEP_ROUNDS = 3
const COMPRESS_HEAD = 300
const COMPRESS_TAIL = 100
const COMPRESS_MAX_FULL = 450
 ...
 * 更早轮次的 tool 结果文本裁剪、媒体删除；assistant 的 tool_calls 参数裁剪
 */
    ...
    if (m.role === "tool") {
      ...
      if (m.tool_calls?.length) {
        for (const tc of m.tool_calls) {
          if (typeof tc.function?.arguments === "string") tc.function.arguments = compressText(tc.function.arguments)
        }
      }
    }
  }
  ...
  async run(entry) {
    ...
    try {
      for (let round = 0; round < maxRounds; round++) {
        ...
        // 滚动压缩历史，保留最近 3 轮完整，更早轮次裁剪工具输入/输出

...
export class AgentRunner {
  ...
  async run(entry) {
    ...
    // Agent 技能列表(config/skills/*/SKILL.md)，无可用技能时不暴露 skill 工具
    const skills = await agentSkills.list()
    const toolDefs = skills.length
      ? agentTools.getDefinitions()
      : agentTools.getDefinitions().filter(t => t.function?.name !== "skill")

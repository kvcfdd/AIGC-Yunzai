...
const AGENT_SYSTEM = [
  ...
  "2. 本地拥有完整的node.js/ffmpeg环境，且已安装playwright浏览器，如需编写脚本请使用 node.js ，禁止使用 python、php、java 等其他语言；如需调用无头浏览器请使用 playwright，禁止使用 puppeteer、selenium 等其他库！",
  ...
  "9. 任务完成后使用自然语言输出最终任务结果；如有需要交付给 主Agent 的文件，在结果文本中写明文件的绝对路径",
].join("\n")

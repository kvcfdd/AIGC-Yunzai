...
async function cleanupStaleAgents() {
  try {
    ...
    if (cleaned > 0) Bot.makeLog("info", `[Agent] 清理 ${cleaned} 个因重启丢失的任务`)
  } catch {}
}
...
export class AgentRunner {
  ...
  async run(entry) {
    ...
    const maxRounds = cfg.aigc?.agent?.max_rounds || 15
        ...
        if (res.tool_calls?.length) {
          ...
          if (this._pendingClarify) {
            ...
            this._fireClarify(clarifyQuestion).catch(err => {
              Bot.makeLog("warn", `[Agent] clarify 注入失败 (${this.taskId}): ${err.message}，降级继续执行`)
    ...
    await this._persist()
    Bot.makeLog("info", `[Agent] 任务 ${this.taskId} 结束, status=${this.status}`)
  }
  ...
  async _waitForResume() {
    ...
    const timer = setTimeout(() => {
      Bot.makeLog("warn", `[Agent] clarify 超时 (${CLARIFY_TIMEOUT_S}s)，自动取消任务 ${this.taskId}`)
      this.cancel()
  ...
  async _fireClarify(question) {
    Bot.aigc
      ...
      .catch(err => Bot.makeLog("warn", `[Agent] clarify inject 失败: ${err.message}`))
  }
  ...
  async _injectDeliver() {
    ...
    try {
      ...
      Bot.makeLog("warn", `[Agent] deliver inject 失败: ${err.message}`)
    }
  }
  ...
  async _injectFailed() {
    try {
      ...
      Bot.makeLog("warn", `[Agent] failed inject 失败: ${err.message}`)
    }
  }
}

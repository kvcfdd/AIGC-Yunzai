  ...
  async run(entry) {
    ...
    try {
      for (let round = 0; round < maxRounds; round++) {
        ...
        const opts = { model: agentModel, max_tokens: agentMaxTokens, tools: toolDefs, signal: this._abortController.signal, channel: "agent" }
        ...
        if (res.tool_calls?.length) {
          ...
          if (round === maxRounds - 1) {
            const finalRes = await Bot.aigc.provider.chat(messages, { model: agentModel, tool_choice: "none", tools: toolDefs, signal: this._abortController.signal, channel: "agent" })
            if (finalRes.blocked) {
              this.status = "failed"

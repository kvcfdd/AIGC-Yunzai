...
const AGENT_SYSTEM = [
  ...
  "2. 本地拥有完整的node.js/ffmpeg环境，且已安装playwright浏览器，如需编写脚本请优先使用 node.js ，需要使用python请根据系统环境抉择(Linux请注意: 请先创建虚拟环境用于安装所需python库,禁止强行安装全局库)；如需调用无头浏览器请使用 playwright，禁止使用 puppeteer、selenium 等其他库！",
  ...
  "9. 维护全局知识库 knowledge.md(项目根目录):发现可复用的经验或知识点时(如虚拟环境、已安装的库、环境配置、踩过的坑)可用 bash 写入(没源文件就自己创建)；遇到相似或重复的经验点直接合并更新，过期或错误的内容可删改，保持文件简洁有效；系统提示中已注入历史经验，重复内容无需再写",
...
export class AgentRunner {
  ...
  async run(entry) {
    ...
    const agentModel = cfg.aigc?.agent?.model
    if (!agentModel) {
      this.status = "failed"
        ...
        if (res.tool_calls?.length) {
          ...
          if (this._pendingClarify) {
            ...
            this._fireClarify(clarifyQuestion).catch(err => {
              ...
              messages.push({ role: "user", content: `[系统提示] 澄清问题 "${clarifyQuestion}" 未能送达 主Agent ，请基于已有信息自行决定下一步操作或直接输出任务失败，缺失了哪些信息。` })
              if (this.status === "waiting") {
                try {
                  this.resume("(请直接输出任务失败，缺失了哪些信息。)")
                } catch {}
              }
            })

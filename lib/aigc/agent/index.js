...
class AgentManager {
  ...
  async submit(entry) {
    ...
    const taskId = String(userId)
    if (!/^[\w-]+$/.test(taskId)) throw new Error(`userId 含非法字符，无法作为任务 ID: ${taskId}`)

    // 并发限制
    let userRunning = 0
    for (const [, r] of runners) {
      if (r._userId === userId && r.status === "running") userRunning++
    }

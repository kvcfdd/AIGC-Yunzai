...
export async function downloadWithAria2c(videoUrl, audioUrl, videoPath, audioPath, headers) {
  ...
  await Promise.all([execFileAsync("aria2c", [...baseArgs, "-o", path.basename(videoPath), "-d", path.dirname(videoPath), videoUrl]), execFileAsync("aria2c", [...baseArgs, "-o", path.basename(audioPath), "-d", path.dirname(audioPath), audioUrl])])

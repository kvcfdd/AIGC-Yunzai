 ...
 * @param {Set<string>} [filterUserIds] — 需要剔除的 QQ 集合，非空时跳过这些用户的发言
 * @returns {string|null}
 */
export function formatGroupHistory(msgs, resolveAtName, selfId, masterCfg, filterUserIds = null) {
  const lines = []
  for (const msg of msgs) {
    const sender = msg.sender || {}
    const name = sender.card || sender.nickname || "Unknown"
    const qq = sender.user_id || "?"
    if (filterUserIds && filterUserIds.has(String(qq))) continue

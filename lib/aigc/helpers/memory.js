...
function buildSummarizePrompt(systemPrompt, qa, memories) {
  ...
  const hardcoded = `============================================
[系统提示] 现在是每日总结时间，来记录一下今天与ta的对话历程吧~

说明: 像记日记一样记录今天聊了什么，关键信息要保留，字数越短越好，最多不超过200字。
输出要求: 直接输出摘要文本本身，不要输出任何前缀、引导语、问候语或解释；不要模仿 <history> 中的"日期: 内容"格式，不要引用任何日期。
...
export async function summarizeOne(self_id, user_id, date) {
  ...
  try {
    ...
    const opts = { stateful: false, retry_count: 2, channel: "ambient", model: ambient.model || "gemini-3-5-flash" }

...
export async function summarizeOne(self_id, user_id, date) {
  ...
  const qa = con().extractQA(session.messages, { userLabel })
  if (!qa) return { ok: false, reason: "无可总结内容" }

  try {
    const ambient = cfg.aigc?.ambient || {}
    if (!ambient.model) {
      log.warn(`记忆总结: 未配置模型 (cfg.aigc.ambient.model)，跳过 [${self_id}/${user_id}]`)
      return { ok: false, reason: "未配置记忆模型" }
    }
    const opts = { stateful: false, retry_count: 2, channel: "ambient", model: ambient.model }

...
import { parseTaggedReply, buildAssistantMsg, userMsgMeta } from "../helpers/message.js"
import { faceName, faceId } from "../helpers/face.js"
...
export class AigcChatCore extends plugin {
  ...
  async _runDialogue(userMsg, isAmbient) {
    ...
    activeRequests.set(this.e.user_id, { controller, isAmbient, pendingMsg: finalMsg, pendingImg: finalImg, pendingVideo: finalVideo, pendingAudio: finalAudio, pendingFiles: finalFiles, group_id: this.e.isGroup ? String(this.e.group_id) : null })
  ...
  _applyMediaPathMarkers(m) {
    const kinds = [
      { list: m.images, paths: m.image_paths, label: "图片" },
      { list: m.videos, paths: m.video_paths, label: "视频" },
      { list: m.audios, paths: m.audio_paths, label: "语音" },
      { list: m.files, paths: m.file_paths, label: "文件" },
    ]
    for (const { list, paths, label } of kinds) {
      ...
      if (m.role === "user") {
        ...
        let idx = 0
        content = content.replace(new RegExp(`\\[${label}\\]`, "g"), () => {
          const p = (paths || [])[idx++]
          return p ? `[${label}](${p})` : `[${label}已过期]`
        })

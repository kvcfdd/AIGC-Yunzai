...
export class AigcChatCore extends plugin {
  ...
  async _runDialogue(userMsg, isAmbient) {
    ...
    if (Array.isArray(this.e.message)) {
      for (const seg of this.e.message) {
        ...
        if (!src) continue
        if (seg.type === "image") finalImg.push(src)
        else if (seg.type === "video") finalVideo.push(src)
        else if (seg.type === "record" || seg.type === "audio") finalAudio.push(src)
        else if (seg.type === "file") {
          const kind = classifyFileExt(seg.name)
          if (kind === "image") finalImg.push(src)
          else if (kind === "video") finalVideo.push(src)
          else if (kind === "audio") finalAudio.push(src)
          else finalFiles.push(seg)
        }
      }
    }
    ...
    try {
      ...
      const imgRes = await provider.resolveImages(finalImg, controller.signal)
  ...
  async _replyLoop(sessionKey, userMsg, imgRes, vidRes, audRes, fileRes, systemPrompt, signal, isAmbient = false) {
    ...
    const toolDefs = this.e._injected
      ? tools()
          .getDefinitions()
    ...
    const firstUserMsg = {
      ...
      ...(fileRes?.files?.length ? { files: fileRes.files, file_paths: fileRes.file_paths } : {}),
    }

  ...
  async _runDialogue(userMsg, isAmbient) {
    ...
    try {
      ...
      for (const t of turns) {
        t.imgRes = await provider.resolveImages(t.imgs, controller.signal)
        t.vidRes = await provider.resolveVideo(t.videos, removeAudio, controller.signal)
        t.audRes = removeAudio ? null : await provider.resolveAudio(t.audios, controller.signal)
        t.fileRes = await provider.resolveFiles(t.files, controller.signal)
      }

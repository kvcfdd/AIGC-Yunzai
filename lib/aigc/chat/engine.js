...
import conversation, { contextRounds } from "../conversation.js"
...
// imgRes.uris 的元素为 string，或超长图切段后的 string[]
export const activeRequests = new Map()

/** activeRequests 的 key — 同时验证 bot(self_id) 与用户 */
export const reqKey = e => `${e.self_id}:${e.user_id}`

/** 单项是否已解析成功 —— 占位符([图像异常] 等)与 null 都判否 */
const mediaResolvedOne = u => typeof u === "string" && u.startsWith("data:")
/** 媒体解析结果是否可直接复用: 全部项编码成功(data URI)，无异常占位符。
 *  打断合并复用 turn 时，被中断请求可能留下占位结果，需重新解析。
 *  图片项可能是数组(超长图切段)，任一段未解析成功即整组重解析；
 *  空数组同样判否 —— 否则会被当作已解析，静默不发任何图片部件 */
const mediaResolved = res => !!res && (res.uris || res.files)?.every?.(u => (Array.isArray(u) ? u.length > 0 && u.every(mediaResolvedOne) : mediaResolvedOne(u)))
...
export class AigcChatCore extends plugin {
  ...
  async _runDialogue(userMsg, isAmbient) {
    ...
    while (true) {
      ...
      for (const t of turns) {
        ...
        try {
          await this._resolveTurnMedia(t)
        } catch (err) {
    ...
    const sid = con().sessionId(this.e.self_id, this.e.user_id)
    ...
    try {
      ...
      for (const t of turns) {
        await this._resolveTurnMedia(t, controller.signal)
      }
      await this._replyLoop(sid, turns, systemPrompt, controller.signal, isAmbient)
    } catch (err) {
  ...
  async _resolveTurnMedia(t, signal) {
    if (!mediaResolved(t.imgRes)) t.imgRes = await provider.resolveImages(t.imgs, signal)
    if (!mediaResolved(t.vidRes)) t.vidRes = await provider.resolveVideo(t.videos, signal)
    if (!mediaResolved(t.audRes)) t.audRes = await provider.resolveAudio(t.audios, signal)
    if (!mediaResolved(t.fileRes)) t.fileRes = await provider.resolveFiles(t.files, signal)
  }

  /** 清理临时标记/媒体编码后原子落盘本轮对话
   *  isAmbient 为 true 时直接不落盘 */
  async _persistRound(sid, localPending, isAmbient = false) {
    if (isAmbient) return
    ...
    await con().appendMessages(sid, localPending)
  }
  ...
  _applyMediaPathMarkers(m) {
    ...
    // 文件名与路径都容忍括号: 原名(如「报告(1).pdf」)会被 safeFileBase 一路带进
    // 缓存路径，用 [^)]* 匹配只会取到第一个 )，占位符替换不成功、本地路径只能
    // 追加到文末，正文里反而留下原始 URL
    const kinds = [
  ...
  /** 思维链重建：子模型按 配置系统提示词 + 近 N 轮历史 + 本轮经过 + 原始英文摘要
  ...
  async _reconstructThinking(res, reconCtx, localPending, signal, tr) {
    ...
    const maxUserMsgs = contextRounds()
    const hist = reconCtx?.history || []
    const recent = []
    for (let i = hist.length - 1; i >= 0; i--) {
      recent.unshift(hist[i])
      if (recent.filter(m => m.role === "user").length >= maxUserMsgs) break
    }
...
${origSummary ? `===== 主模型原始思考摘要(英文，仅作线索参考，不要逐句直译或照抄) =====\n${origSummary}` : ""}`
    ...
    try {
      ...
      const child = await Bot.sleep(
        ...
        provider.chat(
          ...
          { model, channel: "reconstruct", max_tokens: 65536, signal },
        ),
      )
  ...
  async _finishTextReply(res, sid, localPending, signal, isAmbient, replyQuote, reconCtx = null, warnMsg) {
    ...
    if (!text || /^no_reply$/i.test(text)) {
      await this._persistRound(sid, localPending, isAmbient)
      return false
    }
   ...
   *  - localPending[]  最终原子写入 SQLite 的完整对话记录
   *  - apiMessages[]   每轮实际发送给 API 的消息
   *  主动插话：固定使用 ambient.model 分流，可用工具与上下文，但整轮不落盘。 */
  async _replyLoop(sid, userTurns, systemPrompt, signal, isAmbient = false) {
    ...
    const toolDefs = tools()
      ...
      .filter(t => {
        ...
        if (isAmbient && name === "chat_history") return false
        return true
      })
    ...
    for (let round = 0; round < maxRounds; round++) {
      ...
      if (round === 0) {
        // 首轮：系统提示词 + 历史 + 本轮输入
        apiMessages = [systemMsg, ...baseMessages, ...userMsgs]
      } else {
        // 后续工具轮：完整历史 + 本轮累积
        apiMessages = [systemMsg, ...baseMessages, ...localPending]
      }
      ...
      const res = await provider.chat(apiMessages, opts)
      ...
      // 文本回复
      if (res.content) {
        return this._finishTextReply(res, sid, localPending, signal, isAmbient, replyQuote, { system: systemMsg.content, history: baseMessages })
      }
    ...
    const finalMessages = [systemMsg, ...baseMessages, ...localPending]

    const finalOpts = { signal, tool_choice: "none", channel: isAmbient ? "ambient" : "main" }
    ...
    if (finalReply.content) {
      ...
      return this._finishTextReply(finalReply, sid, localPending, signal, isAmbient, replyQuote, { system: systemMsg.content, history: baseMessages }, "工具轮次超限，降级回复成功")
    }
    log.error(`全部失败`)
    // 水群与定时/后台注入静默丢弃
    if (isAmbient || this.e?._injected) return false

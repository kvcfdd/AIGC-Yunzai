...
export class AigcChatCore extends plugin {
  ...
  async _persistRound(sessionKey, localPending, isAmbient = false) {
    if (isAmbient) return
...
${origSummary ? `===== 主模型原始思考摘要(英文，仅作线索参考，不要逐句直译或照抄) =====\n${origSummary}` : ""}`
  ...
  async _finishTextReply(res, sessionKey, localPending, signal, isAmbient, replyQuote, reconCtx = null, warnMsg) {
    ...
    if (!text || /^no_reply$/i.test(text)) {
      await this._persistRound(sessionKey, localPending, isAmbient)
      return false
    }
   ...
   *  主动插话：固定使用 ambient.model 分流，可用工具/记忆/上下文，但整轮不落盘。 */
  async _replyLoop(sessionKey, userTurns, systemPrompt, signal, isAmbient = false) {
    ...
    for (let round = 0; round < maxRounds; round++) {
      ...
      // 文本回复
      if (res.content) {
        return this._finishTextReply(res, sessionKey, localPending, signal, isAmbient, replyQuote, { system: systemMsg.content, history: baseMessages })
      }
    ...
    if (finalReply.content) {
      ...
      return this._finishTextReply(finalReply, sessionKey, localPending, signal, isAmbient, replyQuote, { system: systemMsg.content, history: baseMessages }, "工具轮次超限，降级回复成功")
    }
    log.error(`全部失败`)
    // 水群与定时/后台注入静默丢弃
    if (isAmbient || this.e?._injected) return false

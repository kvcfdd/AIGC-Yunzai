...
export const systemMethods = {
  ...
  _resolveAtName(qq) {
    ...
    try {
      if (this.e?.isGroup) {
        ...
        if (String(qq) === String(this.e?.user_id)) return this.e?.sender?.nickname || this.e?.friend?.nickname || String(qq)
        if (String(qq) === String(this.e?.self_id)) return Bot[this.e?.self_id]?.nickname || String(qq)
      }
    } catch (err) {
      // 避免解析异常导致流程中断
    }
  ...
  /** 获取当前生效的 AIGC 黑名单 QQ 集合 */
  _blacklistSet() {
    return new Set((cfg.aigc?.qq_blacklist || []).map(String))
  },

  /** 构建 system prompt：提示词 + 技能 + 群友上下文 + 环境
   *  @param isAmbient 是否水群模式 */
  async _buildSystem(userMsg, isAmbient = false) {
    const parts = []
    ...
    if (rawHistory) {
      // 水群模式剥离黑名单用户的群友上下文
      const filter = isAmbient ? this._blacklistSet() : null
      const groupUsersCtx = await this._buildGroupUsersContext(rawHistory, filter)
  ...
  async _buildEnvContext(isAmbient = false, rawHistory = null) {
    ...
    if (e.isGroup) {
      ...
      if (isAmbient) {
        lines.push("[系统提示] 本次为系统触发，你可以根据群友们的聊天内容自行决定是否参与聊天，不参与就输出 no_reply 表示已读不回。")
      } else {
      ...
      if (histCount > 0) {
        ...
        if (msgs) {
          // 水群模式剥离黑名单用户的发言，避免 bot 行为割裂
          const filter = isAmbient ? this._blacklistSet() : null
          const history = formatMsgHistory(msgs, this._resolveAtName.bind(this), this.e.self_id, cfg.master, filter)
    ...
    const masterLabel = e.isMaster ? ", bot owner (master)" : ""
    const botPart = `你的QQ: ${e.self_id}`
    let injectedNote = ""
    if (e._injected) {
      injectedNote = `\n[系统提示] 本轮对话由定时任务/后台任务自动触发`
    }

    const lines = [`现在是${formatDate(new Date(), "full")}。`, botPart, `用户信息: ${name}(QQ: ${e.user_id}${masterLabel})${injectedNote}`]

    const msgs = await this._getPrivateHistoryRaw(PRIVATE_HISTORY_COUNT)
    if (msgs) {
      const history = formatMsgHistory(msgs, this._resolveAtName.bind(this), this.e.self_id, cfg.master)
      if (history) lines.push(`<private_history>\n${history}\n</private_history>`)
    }
  ...
  /** 获取与好友的最近 N 条原始消息 */
  async _getPrivateHistoryRaw(count) {
    try {
      const e = this.e
      if (!e.friend?.getChatHistory) return null

      const msgSeq = e.message_seq
      if (!msgSeq) return null

      const msgs = await e.friend.getChatHistory(msgSeq, count, true)
      return msgs?.length ? msgs : null
    } catch (err) {
      log.warn(`私聊记录获取失败: ${err.message}`)
      return null
    }
  },
   ...
   *  @param {Set<string>} [filterUserIds] - 需要剔除的 QQ 集合 */
  async _buildGroupUsersContext(rawHistory, filterUserIds = null) {
    ...
    // 剥离黑名单用户的群友上下文
    if (filterUserIds) {
      for (const qq of filterUserIds) userIds.delete(qq)
    }

...
const SWEEP_SELECT = "SELECT key FROM redis_expires WHERE expires_at <= ? LIMIT ?"
...
class RedisClient {
  ...
  _purge(key) {
    let n = 0
    n += this._stmt("DELETE FROM redis_kv WHERE key = ?").run(key).changes
    n += this._stmt("DELETE FROM redis_hash WHERE key = ?").run(key).changes
    n += this._stmt("DELETE FROM redis_zset WHERE key = ?").run(key).changes
    this._stmt("DELETE FROM redis_expires WHERE key = ?").run(key)
    return n
  }

  /** 键是否还存在 */
  _exists(key) {
    return !!(this._stmt("SELECT 1 FROM redis_kv WHERE key = ?").get(key) || this._stmt("SELECT 1 FROM redis_hash WHERE key = ?").get(key) || this._stmt("SELECT 1 FROM redis_zset WHERE key = ?").get(key))
  }
  ...
  _applyExpire(key, opts) {
    ...
    this._stmt("INSERT INTO redis_expires (key, expires_at) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET expires_at = excluded.expires_at").run(key, Date.now() + Number(sec) * 1000)
    return true
  }
  ...
  sweep(budgetMs = 25) {
    ...
    for (;;) {
      ...
      for (const table of ["redis_kv", "redis_hash", "redis_zset"]) this._stmt(`DELETE FROM ${table} WHERE key IN (SELECT value FROM json_each(?))`).run(list)
      const info = this._stmt("DELETE FROM redis_expires WHERE key IN (SELECT value FROM json_each(?))").run(list)
      total += keys.length
  ...
  close() {
    ...
    try {
      this._stmts.clear()
      const f = this._file
      if (f === ":memory:" || String(f).startsWith("file:")) this._db.close()
    } catch {}
  }
}

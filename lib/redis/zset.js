...
export default {
  ...
  zAdd(key, members) {
    ...
    for (const m of list) {
      ...
      const exists = this._stmt("SELECT 1 FROM zset WHERE key = ? AND member = ?").get(key, member)
      this._stmt("INSERT INTO zset (key, member, score) VALUES (?, ?, ?) ON CONFLICT(key, member) DO UPDATE SET score = excluded.score").run(key, member, score)
      if (!exists) added++
    }
    return added
  },
  ...
  zScore(key, member) {
    ...
    const row = this._stmt("SELECT score FROM zset WHERE key = ? AND member = ?").get(key, String(member))
  ...
  zRevRank(key, member) {
    ...
    const row = this._stmt("SELECT score FROM zset WHERE key = ? AND member = ?").get(key, member)
    if (!row) return null
    return this._stmt("SELECT count(*) AS c FROM zset WHERE key = ? AND (score > ? OR (score = ? AND member > ?))").get(key, row.score, row.score, member).c
  },
  ...
  zRem(key, member) {
    key = String(key)
    this._purgeIfExpired(key)
    const info = this._stmt("DELETE FROM zset WHERE key = ? AND member = ?").run(key, String(member))
  ...
  zRemRangeByScore(key, min, max) {
    ...
    const info = this._stmt("DELETE FROM zset WHERE key = ? AND score >= ? AND score <= ?").run(key, lo, hi)
    this._pruneIfEmpty(key)
    return info.changes
  },
  ...
  zCount(key, min, max) {
    ...
    const lo = parseBound(min, -Infinity)
    const hi = parseBound(max, Infinity)
    return this._stmt("SELECT count(*) AS c FROM zset WHERE key = ? AND score >= ? AND score <= ?").get(key, lo, hi).c
  },
  ...
  _zRangeRows(key, start, stop) {
    ...
    const rows = this._stmt("SELECT member, score FROM zset WHERE key = ? ORDER BY score ASC, member ASC").all(key)
  ...
  _zScoreRows(key, min, max) {
    ...
    const lo = parseBound(min, -Infinity)
    const hi = parseBound(max, Infinity)
    return this._stmt("SELECT member, score FROM zset WHERE key = ? AND score >= ? AND score <= ? ORDER BY score ASC, member ASC").all(key, lo, hi)
  },
}
